...
returning one:
    ...
    from tech_planner.application.settings import Settings, BackendSettings
...
from tech_planner.adapters.driven.config.validation import validate
...
def _execute(path: Path) -> Any:
    spec = importlib.util.spec_from_file_location("tech_planner_user_settings", path)
...
def _extract(module: Any, path: Path) -> Settings:
    ...
    if not isinstance(candidate, Settings):
        raise SettingsError(
            ...
            "Import it with: from tech_planner.application.settings import Settings"
        )
    return candidate
